...
export const isPC = (() => {
    ...
    for (let index = 0; index < Agents.length; index++) {
        if (userAgentInfo.toLocaleLowerCase().indexOf(Agents[index]) > 0) {
            isPC = false
            break;
        }

    }
    return isPC && window.innerHeight > 750
})()
...
export const isAndroid = (() => {
    ...
    for (let index = 0; index < AndroidAgents.length; index++) {
        if (userAgentInfo.toLocaleLowerCase().indexOf(AndroidAgents[index]) > 0) {
            isAndroid = true
            break;
